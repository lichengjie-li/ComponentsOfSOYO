...
public class VirtualJoystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    {
        if (GameManager.GMInstance.maskControl.ShowMask < 1 || GameManager.GMInstance.maskControl.ShowMask >= 3)
            GameManager.GMInstance.maskControl.ShowMask = 1;
        isDrag = false;
        if (!GameManager.GMInstance.isFinish) GameManager.GMInstance.isFire = true;
        if (!drag)
        {
