...
public class VirtualJoystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    /// 摇杆的默认位置
    /// </summary>
    private Vector2 Sticktemporary => Adapter.SetAdapterValue(CameraAdapter.GetInstance.Ratio < 1,
    ...
    private Vector2 JoyStickScale(int i)
    {
        if (Screen.width > Screen.height)
            return new Vector2(parentCanvasSize.y / (4 * i), parentCanvasSize.y / (4 * i));
        else
            return new Vector2(parentCanvasSize.x / (3 * i), parentCanvasSize.x / (3 * i));
    }
